...
public class partidosDAO {
    ...
    public List<Partidos> listadoPartidos() {
        //Devuelve una lista de partidos
...
//    }
    ...
    public Partidos buscarPartido(int id) {
        //devuelve un partido buscado por su id
        session = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
    ...
    public void addJuegan(Juegan j) {
        //Registra un Juegan en la base de datos
        session = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        session.save(j);
        tx.commit();
    ...
    public List<Instalaciones> listadoInstalaciones() {
        //Devuelve una lista de instalaciones relacionadas con los partidos y juegan, de modos que para el listado de partidos se muestren correctamente
        session = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
